...
def scoring_function(mod_name, hyperparams):
    ...
    mod_instance = mod_class(metadata, 
                             save_path=f'../outputs/synthesizers/tuning_tot_2/{ind}.pkl',
                             epochs=500,
                             batch_size=10000,
                             cuda='cuda:1',
                             patience=50,
    ...
    # restore best model
    mod_instance = synths[mod_name].load(f'../outputs/synthesizers/tuning_tot_2/{ind}.pkl')
    ind += 1
    scores = []
    for _ in range(10): # Repeat 10 times to mitigate randomness
        synthetic_data = mod_instance.sample(len(y_train) // 10)
    ...
    append_to_pickle_file('../outputs/synthesizers/hyperparams_tot_2.pkl', hyperparams_log)
    append_to_pickle_file('../outputs/synthesizers/scores_tot_2.pkl', scores)
...
with open('../outputs/synthesizers/session_tot_2.pkl', "wb") as f:
